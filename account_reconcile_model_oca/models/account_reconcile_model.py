...
class AccountReconcileModel(models.Model):
    ...
    def _get_invoice_matching_amls_candidates(self, st_line, partner):
        ...
        params_pattern = re.search(r"with params: (.+)>", str(query))
        if params_pattern:
            params = params_pattern.group(1)
        ...
        if tokens:
            ...
            for table_alias, field in (
                ...
                sub_queries.append(
                    ...
                    WHERE {query.where_clause} AND {table_alias}.{field} IS NOT NULL
                """
                )
