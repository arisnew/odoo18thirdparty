...
export class ReconcileController extends KanbanController {
  ...
  async updateJournalInfo() {
    ...
    this.state.currency = (result[0].currency_id || result[0].company_currency_id)[0];
  }
  ...
  async selectRecord(record) {
    ...
    if (record === undefined && this.props.resId) {
      ...
      var records = this.model.root.records.filter((modelRecord) => !modelRecord.data.is_reconciled || modelRecord.data.to_check);
      if (records.length === 0) {
